module.exports = class Handler {
  ...
  // 解析当前帧状态
  getState(data) {
    const data01 = data[0].toString(2), data02 = data[1].toString(2);//第一个字节和第二个字节的二进制字符串
  ...
  getData(data, callback) {
    ...
    if (this.state.remains === 0) {
      const buf = Buffer.concat(this.dataList, this.state.payloadLength);
      //使用掩码maskingKey解析所有数据
      const result = this.parseData(buf);
      // 数据接收完成后回调回业务函数
  ...
  // 解析本次message所有数据
  parseData(allData) {
    const { length: len } = allData;
    let i = 0;
    for (;i < len; i += 1) {
      // 异或运算，使用maskingKey四个字节轮流进行计算
  ...
  // 组装数据帧
  createData(data) {
    const dataType = Buffer.isBuffer(data);// 数据类型
    let dataBuf; // 需要发送的二进制数据
    let dataLength;// 数据真实长度
    let dataIndex = 2; // 数据的起始长度
    let frame; // 数据帧
